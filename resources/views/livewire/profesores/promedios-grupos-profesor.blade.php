<div class="overflow-x-auto bg-white rounded shadow p-6 mb-8">{{ $gruposSelector }}
    <livewire:grupos-selector-profesor :grupos="$gruposSelector" wire:model="grupoSeleccionado" />
    ...
    <div class="space-y-10">
            ...
            @foreach($grupos as $grupo)
                <div>
                    <h4 class="text-lg font-semibold text-green-700 mb-2">
                        Grupo: {{ $grupo->nombre }} - Sección: {{ $grupo->seccion }}
                    </h4>
                                ...
                                @foreach($rubro->preguntas as $preguntaIndex => $pregunta)
                                    <tr>
                                        ...
                                        <td class="border px-2 py-1 text-center">
                                            @php
                                                $prom = $promedios[$grupo->id][$rubro->id][$pregunta->id] ?? null;
                                            @endphp
                                            @if($prom !== null)
                    ...
                    <div class="text-sm md:text-base text-gray-700 mt-2">
                        ...
                        @php
                            ...
                            foreach($rubros as $rubro) {
                                foreach($rubro->preguntas as $pregunta) {
                                    foreach(App\Models\Resultado::whereHas('inscripcion', function($q) use ($grupo) {
        ...
        @elseif($grupoSeleccionado)
            @php
                $grupoSel = $grupos->firstWhere('id', (int)$grupoSeleccionado);
            @endphp
            @if($grupoSel)
                <div>
                    <h4 class="text-lg font-semibold text-green-700 mb-2">
    ...
    <div class="text-red-600 text-xs mb-2">Debug: grupoSeleccionado = '{{ $grupoSeleccionado }}'</div>
</div>
