...
@section('content')
    <div class="w-full max-w-lg max-w-screen-md mx-auto pt-10 animate-fade animate-ease-in-out animate-duration-1000">
        ...
        <form class="max-w-sm mx-auto mt-5" method="POST" action="{{ route('contacto.enviar') }}">
            ...
            <button type="submit"
                    class="text-white bg-teal-600 hover:bg-teal-500 focus:ring-4 focus:outline-none focus:ring-blue font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center mt-5">
                Enviar
            </button>
        </form>
    </div>
    ...
    <script>
        grecaptcha.ready(function() {
            grecaptcha.execute('{{ env('RECAPTCHA_SITE_KEY') }}', {action: 'contacto'}).then(function(token) {
